...
public class MetodosOrdenamiento {
    ...
    // cambiar (retunr new int [] {}) por return arreglo
    ...
    // Método de burbuja tradicional con errores
    // Error encontrado:
    // en for (int i = 0; i < n - 1 ; i++) cambiado por for (int i = 0; i < n; i++)
    // tambien en for (int j = 0; j < n; j++) cambiado por for (int j = i + 1; j <
    // n; j++)
    // int temp = arreglo[j]; cambiado por int temp = arreglo[i]
    // arreglo[j] = arreglo[j]; cambiado por arreglo[i] = arreglo[j]
    // arreglo[j + 1] = temp cambiado por arreglo[j] = temp
    ...
    // Error encontrado:
    // no tenia return arreglo, no se imprimian los valores
    // agregamos rerturn arreglo; al finalizar el primer for
    ...
    // Método de selección con errores
    // Error encontrado:
    // en for (int i = 0; i < arreglo.length ; i++) hacia falta - 1, corregido for
    // (int i = 0; i < arreglo.length - 1; i++)
    // en for (int j = i + 1; j < arreglo.length; j--) lo corregimos por for (int j
    // = i + 1; j < arreglo.length; j++)
    ...
    // Método de selección con errores
    // Error encontrado:
    // en int smallerNumber = arreglo[i] lo remplazamos por int smallerNumber
    // =arreglo[indiceMinimo]
    // en arreglo[indiceMinimo] = smallerNumber lo reemplazamos por arreglo[i]
    // =smallerNumber
    ...
    // Error encontrado:
    // en while (i > 0 && arreglo[i] > key) hacia falta darle el igual a 0 while (i
    // >= 0 && arreglo[i] > key)
